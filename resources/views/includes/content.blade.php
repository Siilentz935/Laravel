...
<div class="flex items-center justify-center h-screen">
  ...
  <div class="m-2 p-4">
    <form action="{{route('login.submit')}}" method="post" class="bg-gray-100 p-6 rounded-md">
      {{csrf_field()}}
      ...
      <div>
        <button type="submit" class="bg-teal-500 text-white py-2 px-4 rounded-md focus:outline-none hover:bg-teal-600" onclick="showLoading()">Ingresar</button>
        <span id="loadingMsg" class="text-teal-500" style="display: none;">Iniciando sesión...</span>
      </div>
    </form>
  </div>
</div>
